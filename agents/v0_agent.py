...
2. 用 Claude 分析哪些页面区域需要改进，生成改进后的组件代码
3. 通过 GitHub API 推送到 main 分支，Vercel 自动部署

如果数据不足（冷启动），Claude 使用最佳实践策略主动优化。
如果改动导致指标下降，下一轮 Agent 会读到数据并自动修正。
...
def fetch_posthog_insights() -> dict:
    ...
    pageviews_resp = requests.get(
        f"{base_url}/events/?event=$pageview&limit=100", headers=headers
    )
    custom_resp = requests.get(
        f"{base_url}/events/?limit=200", headers=headers
    )
...
def analyze_and_generate(insights: dict) -> dict:
    ...
    event_summary = (
        json.dumps(insights["custom_events"], ensure_ascii=False)
        if insights["custom_events"] else "暂无数据"
    )
...
直接返回改进后的完整 hero.tsx 代码："""
    ...
    return {
        ...
        "improved": code,
        "analysis": (
            f"基于 {insights['pageviews']} 次浏览，"
            f"{'冷启动策略' if insights['cold_start'] else '数据驱动'}优化 Hero 文案"
        )
    }
...
def push_to_github(result: dict) -> bool:
    ...
    commit_message = (
        f"feat(agent): 内循环优化 Hero 组件 [{now}]\n\n{result['analysis']}"
    )
