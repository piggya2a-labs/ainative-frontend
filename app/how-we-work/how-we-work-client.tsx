...
export default function HowWeWorkClient() {
  ...
  useEffect(() => {
    posthog?.capture('page_view', { page: 'how_we_work', template: 'customer_success' })
  }, [posthog])
  ...
  return (
    <main className="flex-1 max-w-5xl mx-auto px-4 pt-24 pb-16 w-full">
      ...
      <div className="mb-8 space-y-2">
        ...
        <h1 className="text-3xl font-bold tracking-tight">我们怎么一起工作</h1>
        <p className="text-muted-foreground text-sm max-w-2xl">
          ONIT 的两份公开交付文档——这是我们与你们一起追同一个目标的工作方式。
          共同成功计划是完整操作系统，实施进度表是每天看的施工进度牌。Agent 照着填，以后打包成 Skill。
        </p>
      </div>

      <Tabs defaultValue="mcsp" onValueChange={(v) => posthog?.capture('how_we_work_tab_switch', { tab: v })}>        <TabsList className="mb-6">
